...
def run_end2end_GPT4o(questions, answers, engine, info=False):
...
if __name__ == '__main__':
    ...
    judge_scores, judge_accs, outputs = run_end2end_GPT4o(data['question'], data['model_completion'], judge_name, info=False)

    avg_judge_score = sum(judge_scores) / len(judge_scores)
    avg_judge_acc = sum(judge_accs) / len(judge_accs)

    print("Average truth accuracy:\n" + f"{avg_judge_acc:.10f}")

    output_file = "resul_truth.json"

    with open(output_file, 'w') as f:
        json.dump({'judge_scores': judge_scores,
